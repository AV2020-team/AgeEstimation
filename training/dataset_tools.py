...
def draw_emotion(y, w, h, emotion_labels=None):
    ...
    for i, yi in enumerate(y):
        ...
        cv2.putText(emotionim, "%s: %.1f" % (EMOTIONS[emoindex], yi), (0, i * 20 + 14), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                    (255, 255, 255))
    return emotionim
...
def face_alignment(img, shape_predictor):
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

    # Find the (5)68 face landmarks we need to do the alignment.
    faces = dlib.full_object_detections()

    full_rect = dlib.rectangle(
        0,
        0,
        img.shape[1],
        img.shape[0]
    )
    faces.append(shape_predictor(gray, full_rect))
    return dlib.get_face_chip(img, faces[0], size=max(img.shape[:2]))
...
class DataGenerator(tensorflow.keras.utils.Sequence):  # TODO VIGILANTE
    ...
    def __init__(self, data, target_shape, with_augmentation=True, batch_size=64, custom_augmentation=None,
                 num_classes=None, preprocessing='full_normalization', fullinfo=False, hdf=None,
    ...
    def _preprocessing(self, frame, roi):
